...
/** Uses Gemini URL Context and Google Search for the user-submitted public link only. */
export function createGeminiLinkExtractor(apiKey: string): LinkExtractor {
  return {
    async extract(url, evidence) {
      ...
      if (!res.ok) throw new Error(`Gemini 추출 요청 실패: ${res.status} ${await res.text()}`);
      const text = responseText(await res.json() as { candidates?: Array<{ content?: { parts?: Array<{ text?: string }> } }> });
      let parsed: unknown;
