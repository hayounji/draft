...
export function createOpenAiLinkExtractor(apiKey: string): LinkExtractor {
  return {
    async extract(url) {
      ...
      let parsed: unknown;
      try { parsed = JSON.parse(text); } catch { throw new Error("AI가 유효한 JSON 추출 결과를 반환하지 않았습니다."); }
