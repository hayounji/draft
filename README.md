...
공개 SNS 링크 하나를 받아 AI가 장소명·지역·분류·메뉴/테마 키워드(예: `팬케이크`, `성수동`)를 추출하고, 결과를 검증해 Notion DB에 저장합니다. 네이버지도 관심장소 저장은 이미 로그인된 **로컬 Chrome 프로필**을 Playwright로 열며, 명시적 승인 없이는 실행되지 않습니다.
